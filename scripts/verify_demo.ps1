...
$CategoryConsistent = $null -ne $Current -and $null -ne $Forecast -and $null -ne $Advisory -and $null -ne $Snapshot -and $Current.data.category -eq $Advisory.data.category -and $Current.data.category -eq $Snapshot.current.category -and $Forecast.data.peak.category -eq $Snapshot.forecast.peak.category
$PriorityConsistent = $null -ne $Intelligence -and $null -ne $Actions -and $null -ne $Forecast -and $null -ne $Snapshot -and $Intelligence.data.priority -eq $Snapshot.intelligence.priority -and $Forecast.data.priority -eq $Snapshot.forecast.priority -and @($Actions.data | Where-Object { $_.priority -ne $Intelligence.data.priority }).Count -eq 0 -and @($Snapshot.actions | Where-Object { $_.priority -ne $Intelligence.data.priority }).Count -eq 0
