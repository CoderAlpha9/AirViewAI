...
def _bearing(
    ...
    return (
        np.degrees(
            np.arctan2(
                np.sin(delta) * np.cos(lat2),
                np.cos(lat1) * np.sin(lat2) - np.sin(lat1) * np.cos(lat2) * np.cos(delta),
            )
        )
        + 360
    ) % 360
...
def _issues() -> pd.DataFrame:
    ...
    times = attribution[
        ["city_id", "station_id", "timestamp_utc", "latitude", "longitude"]
    ].drop_duplicates()
    ...
    return times.merge(
        weather[["station_id", "timestamp_utc", "wind_speed_10m", "wind_direction_10m"]],
        on=["station_id", "timestamp_utc"],
        how="left",
    ).sort_values(["city_id", "timestamp_utc"])
...
def build() -> pd.DataFrame:
    ...
    for (city, station), group in issues.groupby(["city_id", "station_id"], sort=False):
        first = group.iloc[0]
        local = events[
            (events.latitude.between(first.latitude - 3, first.latitude + 3))
            & (events.longitude.between(first.longitude - 3, first.longitude + 3))
        ].copy()
        distances = _distance(
            local.latitude.to_numpy(), local.longitude.to_numpy(), first.latitude, first.longitude
        )
        ...
        for issue in group.itertuples(index=False):
            result: dict[str, object] = {
                ...
                "wind_speed_10m": issue.wind_speed_10m,
                "wind_direction_10m": issue.wind_direction_10m,
                "firms_data_available": True,
            }
            issue_time = issue.timestamp_utc.to_datetime64()
            for hours in WINDOWS:
                start = issue_time - np.timedelta64(hours, "h")
                left, right = (
                    np.searchsorted(timestamps, start, side="left"),
                    np.searchsorted(timestamps, issue_time, side="right"),
                )
                ...
                age = (
                    (issue.timestamp_utc - subset.timestamp_utc).dt.total_seconds().to_numpy()
                    / 3600
                    if len(subset)
                    else np.array([])
                )
                align = (
                            ...
                            np.radians(
                                np.abs(
                                    (
                                        bearings[left:right]
                                        - float(
                                            issue.wind_direction_10m
                                            if pd.notna(issue.wind_direction_10m)
                                            else 0
                                        )
                                        + 180
                                    )
                    ...
                    if len(subset) and pd.notna(issue.wind_direction_10m)
                    else np.zeros(len(subset))
                )
                result[f"firms_count_{hours}h"] = int(len(subset))
                result[f"firms_frp_{hours}h"] = float(frp.sum())
                result[f"firms_nearest_km_{hours}h"] = (
                    float(distance.min()) if len(distance) else np.nan
                )
                result[f"firms_max_confidence_{hours}h"] = (
                    float(pd.to_numeric(subset.confidence, errors="coerce").max())
                    if len(subset)
                    else np.nan
                )
                result[f"firms_upwind_count_{hours}h"] = int((align > 0.5).sum())
                result[f"firms_upwind_frp_{hours}h"] = float(frp[align > 0.5].sum())
                result[f"firms_wind_aligned_influence_{hours}h"] = float(
                    (frp * align * np.exp(-distance / 100)).sum()
                )
                result[f"firms_event_age_weighted_influence_{hours}h"] = (
                    float((frp * np.exp(-age / 24) * np.exp(-distance / 100)).sum())
                    if len(subset)
                    else 0.0
                )
                for band in BANDS:
    ...
    (REPORTS / "firms_temporal_feature_report.json").write_text(
        json.dumps(
            {
                "status": "success",
                "row_count": len(output),
                "issue_timestamp_count": len(output),
                "cities": sorted(output.city_id.unique().tolist()),
                "stations": sorted(output.station_id.unique().tolist()),
                "windows_hours": list(WINDOWS),
                "distance_bands_km": list(BANDS),
                "percentage_24h_with_firms_evidence": round(coverage * 100, 4),
                "causality": "events are selected only where event timestamp_utc <= intelligence issue timestamp_utc",
            },
            indent=2,
        ),
        encoding="utf-8",
