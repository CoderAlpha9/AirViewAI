...
class Sentinel5PAdapter:
    ...
    def credentials_status(self) -> SourceResult:
        ...
        return SourceResult(
            ...
            value_kind="provider_derived",
            warnings=[
                "Credentials present; request requires an explicit configured city AOI and date."
            ],
        )
    ...
    def access_token(self) -> str:
        ...
        self._token_expires_at = datetime.now(timezone.utc) + timedelta(
            seconds=int(payload.get("expires_in", 300)) - 30
        )
        return self._access_token

    def validate_statistics(
        self, aoi_geojson: dict[str, Any], start: str, end: str
    ) -> tuple[dict[str, Any], SourceResult]:
        if not self.client_id or not self.client_secret:
        ...
        try:
            ...
            return {}, SourceResult(
                ...
                official_url=self.official_url,
                credential_requirement="COPERNICUS_CLIENT_ID and COPERNICUS_CLIENT_SECRET",
                error=f"{exc}; provider detail: {detail}",
            )
        return payload, SourceResult(
            source=self.source,
            status="success",
            retrieved_at_utc=datetime.now(timezone.utc),
            row_count=len(payload.get("data", [])) if isinstance(payload, dict) else 0,
            official_url=self.official_url,
            credential_requirement="COPERNICUS_CLIENT_ID and COPERNICUS_CLIENT_SECRET",
            processing_steps=[
                "OAuth client-credentials authentication",
                "Copernicus Data Space Statistical API request",
            ],
        )

    @staticmethod
    def statistical_request(
        aoi_geojson: dict[str, Any], start: str, end: str, collection: str = "sentinel-5p-l2"
    ) -> dict[str, Any]:
        return {
            "input": {
                "bounds": {"geometry": aoi_geojson},
                "data": [
                    {"type": collection, "dataFilter": {"timeRange": {"from": start, "to": end}}}
                ],
            },
            "aggregation": {
