...
class OsmAdapter:
    ...
    def fetch_city(
        ...
        return elements, SourceResult(
            source=self.source,
            status="success",
            retrieved_at_utc=datetime.now(timezone.utc),
            row_count=len(elements),
            cache_path=str(cache_path),
            geographic_coverage="configured city AOI",
            official_url=self.official_url,
            licence="ODbL",
            value_kind="proxy",
            processing_steps=["cached conservative Overpass query", "tag extraction"],
        )
    ...
    def tag_features(elements: list[dict[str, Any]]) -> dict[str, int]:
        ...
        return {
            "healthcare_location_count": sum(
                tag.get("amenity") in {"hospital", "clinic", "nursing_home"} for tag in tags
            ),
            "education_location_count": sum(
                tag.get("amenity") in {"school", "college", "university"} for tag in tags
            ),
