...
def validate_air_quality(records: Iterable[AirQualityRecord]) -> list[DataQualityIssue]:
    ...
    for record in records:
        ...
        if not record.station_id:
            issues.append(
                DataQualityIssue(
                    "missing_station_id",
                    "warning",
                    "Observation has no station identity",
                    record.source,
                )
            )
        if record.latitude is None or record.longitude is None:
            issues.append(
                DataQualityIssue(
                    "invalid_coordinates",
                    "warning",
                    "Observation has no valid coordinates",
                    record.source,
                )
            )
        if record.value_original is not None and record.value_original < 0:
            issues.append(
                DataQualityIssue(
                    ...
                    "error",
                    "Pollutant values cannot be negative",
                    record.source,
                )
            )
        ...
        if (
            ...
            and record.value_canonical is not None
            and record.value_canonical > threshold
        ):
            issues.append(
                DataQualityIssue(
                    "physically_implausible_high_value",
                    "warning",
                    "Value retained but exceeds conservative screening threshold",
                    record.source,
                )
            )
        if "unsupported_or_incompatible_unit" in record.quality_flags:
            issues.append(
                DataQualityIssue(
                    "unsupported_units",
                    "warning",
                    "Original value retained; canonical conversion intentionally omitted",
                    record.source,
                )
            )
        ...
        if count > 1:
            issues.append(
                DataQualityIssue(
                    ...
                    "warning",
                    f"Duplicate observation identity appears {count} times",
