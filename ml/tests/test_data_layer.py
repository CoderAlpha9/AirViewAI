...
def test_cpcb_parser_preserves_original_and_flags_invalid_coordinates() -> None:
    rows = CpcbAdapter.parse_records(
        [{"country": "IN", "city": "Delhi", "station": "A", "latitude": "120", "longitude": "77", "pollutant_id": "PM2.5", "pollutant_avg": "12.5", "pollutant_unit": "ug/m3", "last_update": "2026-01-01T00:00:00+05:30"}]
    )
...
def test_open_meteo_parser_uses_utc_and_modelled_label() -> None:
    rows = OpenMeteoAdapter.parse_hourly({"timezone": "UTC", "hourly_units": {"temperature_2m": "°C"}, "hourly": {"time": ["2026-01-01T00:00"], "temperature_2m": [25]}}, "city")
...
def test_sentinel_request_and_missing_credentials() -> None:
    ...
    request = adapter.statistical_request({"type": "Polygon", "coordinates": []}, "2026-01-01T00:00:00Z", "2026-01-02T00:00:00Z")
...
def test_quality_duplicate_and_negative_flags() -> None:
    record = AirQualityRecord("test", "test", "IN", None, None, None, "station", "station", None, 20, 77, "pm10", -1, "ug/m3", -1, "ug/m3", datetime.now(timezone.utc), None, "UTC", datetime.now(timezone.utc), True)
...
def test_grid_is_deterministic_and_wgs84() -> None:
    grid = generate_grid("test", Polygon([(77, 28), (77.01, 28), (77.01, 28.01), (77, 28.01)]), 1_000)
